...
//  getter
  ...
  Future<bool> delivery(
      ...
      String discount,
      String country) async {
    await _firestore.collection("delivery").doc().set({
      "name": name,
      "number": number,
      "transportname": transportname,
      "dateoforder": dateoforder,
  ...
  Future _onStateChanged(User user) async {
    if (user == null) {
      ...
      // addresses = await _addressServices.getUserAddress(uid: user.uid);
...
//      }
  ...
  getAddress() async {
    addresses = await _addressServices.getUserAddress(userId: _user.uid);
    notifyListeners();
  }
